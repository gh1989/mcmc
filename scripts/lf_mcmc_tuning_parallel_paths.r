...
setwd(sprintf("C:/cygwin64/home/Gregg/mcmc/output/%s/1 param",tune_variable))
...
plot(var_x, real_var, log='xy', xlab='Number of Chains', ylab="Posterior Variance", main="", pch=2, col="red")
points(var_x, imag_var, log='xy', main="Im", pch=5, col="blue")

legend("topright", c("Re","Im"), xpd=TRUE, inset=c(-0.5,0), lty=1, col=c('red', 'blue'), bty='n', cex=.75, pch=c(2,5))

###################################
# ESS                             #
###################################

plot(var_x, real_ess, log='xy', xlab='Number of Chains', ylab="ESS", main="", pch=2, col="red")
...
legend('topright',as.character(vals), bty='n', 
        cex=.75, lty=c(1, density_plots), xpd=TRUE, inset=c(-0.5,0) )
...
legend('topright',as.character(vals), bty='n', 
       cex=.75, xpd=TRUE, inset=c(-0.5,0), lty=c(1, density_plots) )


#mtext("Parallel Chains", outer=TRUE, cex=1.5)
